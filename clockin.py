...
fmt = logging.Formatter(fmt = "%(asctime)s %(filename)s:%(lineno)s %(levelname)s %(message)s", datefmt="[%Y-%m-%d %X]")
sh.setFormatter(fmt) 
...
class ZJULogin:
    ...
    def _init_check(self):
        ...
        """
        logger.info("connect to the url page...")
        resp = self.session.get(self.url_login)

        if resp.status_code == 200:
            logger.info("successful 200 OK")
        else:
